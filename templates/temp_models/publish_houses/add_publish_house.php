       ...
       <div class="col-12 card card-warning">
          ...
          <?php
            if($_SERVER['REQUEST_METHOD']=='POST'){
              $name=$_POST['name-publish-house'];
   if(!empty($name)){
       $query = "INSERT INTO publish_houses(id,name_publish_house) Values ('','$name')";
            
        $result = mysqli_query($connection, $query);
              if($result){
                echo '<div class="card-header bg-green">
                <h3 class="card-title">added publish-house</h3>
            </div>';
              }else{

          echo '<div class="card-header bg-red">
                <h3 class="card-title">canot add publish-house</h3>
            </div>';
              }
        ...
        echo '<div class="card-header bg-red">
                <h3 class="card-title">publish-house is empty</h3>
            </div>';
}
